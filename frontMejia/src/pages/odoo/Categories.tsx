...
/**
 * Helper component for consistent row styling in cards.
 */
function Row({
  label,
  value,
...
export default function Categories() {
  ...
  useEffect(() => {
    ...
    const fetchData = async () => {
      ...
      try {
        const response = await axios.get("http://localhost:8000/api/odoo/categories/", {
          signal: controller.signal,
        });

        /**
         * Suponemos que el backend puede devolver el array directamente (patrón Odoo)
         * o envuelto en { data: { categories: [...] } } (patrón PrestaShop).
         */
        ...
        {categories.map((category, i) => (
          ...
          >
            ...
            >
              ...
              <Row
                label="Parent ID"
                value={category.parent_id ?? "Sin padre"}
                color={letterColors[i % letterColors.length]}
              />
            </div>
          </div>
